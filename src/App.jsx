...
const App = () => {
  ...
  return (
    <div className="h-screen w-screen bg-black text-white relative flex flex-col overflow-hidden select-none">
      {/* Top Progress Bar */}
      <div className="absolute top-0 left-0 w-full z-50 flex gap-1 px-2 sm:px-4 py-3 sm:py-6">
        {slides.map((_, i) => (
          <div
            key={i}
            className={`h-0.5 sm:h-1 flex-1 rounded-full transition-all duration-300 ${i <= currentSlide ? 'bg-accent shadow-[0_0_8px_rgba(165,241,78,0.5)]' : 'bg-white/10'}`}
          />
        ))}
      </div>
      ...
      <main className="flex-1 relative flex flex-col overflow-hidden">
        <AnimatePresence mode="wait">
          <motion.div
            ...
            className="flex-1 flex max-h-[calc(100vh-80px)] sm:max-h-[calc(100vh-128px)] slide-container"
          >
      ...
      <footer className="h-16 sm:h-32 px-4 sm:px-12 py-3 sm:pb-8 flex justify-between items-center z-50 border-t border-white/5 bg-black/90 backdrop-blur-xl">
        <div className="scale-75 sm:scale-100 origin-left">
          <Logo />
        </div>
        <div className="flex gap-2 sm:gap-8 items-center">
          <span className="hidden sm:inline text-white/40 font-mono text-xs tracking-[0.4em] uppercase bg-white/5 px-4 py-2 rounded-full border border-white/10">{String(currentSlide + 1).padStart(2, '0')} — {String(slides.length).padStart(2, '0')}</span>
          <span className="sm:hidden text-white/40 font-mono text-[10px] tracking-wider">{String(currentSlide + 1).padStart(2, '0')}/{String(slides.length).padStart(2, '0')}</span>
          <div className="flex gap-2 sm:gap-4">
            <button
              onClick={prevSlide}
              className="p-2 sm:p-4 rounded-full border border-white/5 hover:bg-white/5 transition-all active:scale-90"
            >
              <ChevronLeft size={16} className="sm:w-5 sm:h-5 opacity-40" />
            </button>
            <button
              onClick={nextSlide}
              className="p-2 sm:p-4 rounded-full border border-accent/20 bg-accent/5 hover:bg-accent/10 transition-all active:scale-90 group"
            >
              <ChevronRight size={16} className="sm:w-5 sm:h-5 text-accent group-hover:translate-x-1 transition-transform" />
            </button>
          </div>
        </div>
      </footer>
...
const renderSlide = (slide, exportToPDF) => {
  switch (slide.type) {
    case 'cover':
      return (
        <div className="flex-1 relative slide-wrapper">
          <div className="absolute inset-0 opacity-75">
            <img src={slide.bgImage} className="w-full h-full object-cover brightness-110" alt="Background" />
            <div className="absolute inset-0 bg-gradient-to-r from-black via-black/60 sm:via-black/40 to-black/40 sm:to-black/20" />
          </div>
          <div className="relative z-10 max-w-4xl">
            ...
            >
              <h1 className="text-4xl sm:text-6xl md:text-8xl font-black tracking-tighter leading-none mb-4 sm:mb-6">
              ...
              <p className="text-base sm:text-xl md:text-2xl font-medium text-white/70 mb-4 sm:mb-8">{slide.subtitle}</p>
              <p className="text-xs sm:text-sm md:text-base font-bold tracking-[0.3em] sm:tracking-[0.5em] uppercase opacity-30">{slide.tagline}</p>
            </motion.div>
          </div>
        </div>
      );
    ...
    case 'problem':
      return (
        <div className="flex-1 flex" style={{ padding: 0 }}>
          <div className="w-full sm:w-3/5 slide-wrapper flex flex-col justify-center">
            <h2 className="text-2xl sm:text-4xl md:text-5xl lg:text-7xl font-black mb-6 sm:mb-12 tracking-tighter leading-tight">{slide.title}</h2>
            <div className="space-y-4 sm:space-y-6">
              {slide.points.map((pt, i) => (
                <motion.div
                  ...
                  className="group"
                >
                  <h3 className="text-lg sm:text-xl md:text-2xl font-bold text-accent mb-1 sm:mb-2">{pt.title}</h3>
                  <p className="text-sm sm:text-base md:text-lg text-white/50 group-hover:text-white/90 transition-colors leading-relaxed">{pt.desc}</p>
                  <div className="mt-3 sm:mt-4 h-px w-full bg-white/15 group-last:hidden" />
                </motion.div>
              ))}
            </div>
          </div>
          <div className="hidden sm:block w-2/5 relative h-full">
    ...
    case 'comparison':
      return (
        <div className="flex-1 slide-wrapper overflow-y-auto sm:overflow-visible">
          <div className="slide-content-area">
            <h2 className="text-2xl sm:text-4xl md:text-5xl lg:text-7xl font-black mb-4 sm:mb-6 tracking-tighter text-center">{slide.title}</h2>
            {slide.subtitle && (
              <p className="text-sm sm:text-lg md:text-xl text-white/50 text-center mb-6 sm:mb-12 max-w-4xl mx-auto leading-relaxed px-4">{slide.subtitle}</p>
            )}
            <div className="flex flex-col sm:flex-row gap-4 sm:gap-8 sm:h-[340px]">
              <div className="flex-1 bg-white/[0.04] rounded-2xl sm:rounded-[32px] p-4 sm:p-8 md:p-10 border border-white/[0.12]">
                <h3 className="text-[10px] sm:text-xs font-bold mb-4 sm:mb-8 opacity-50 uppercase tracking-[0.2em] sm:tracking-[0.3em]">{slide.old.title}</h3>
                <ul className="space-y-2 sm:space-y-4">
                  {slide.old.points.map((pt, i) => (
                    <li key={i} className="flex items-center gap-2 sm:gap-3 text-white/50 text-xs sm:text-base">
                      <div className="w-1 h-1 sm:w-1.5 sm:h-1.5 rounded-full bg-white/30 flex-shrink-0" />
                      <span className="line-clamp-2 sm:line-clamp-none">{pt}</span>
                    </li>
                  ))}
                </ul>
              </div>
              <div className="flex-1 bg-accent/5 rounded-2xl sm:rounded-[32px] p-4 sm:p-8 md:p-10 border border-accent/20 relative overflow-hidden group">
                <div className="absolute top-0 right-0 p-4 sm:p-8 grayscale opacity-20 group-hover:grayscale-0 group-hover:opacity-100 transition-all">
                  <Zap className="text-accent animate-pulse" size={16} />
                </div>
                <h3 className="text-[10px] sm:text-xs font-bold mb-4 sm:mb-8 text-accent uppercase tracking-[0.2em] sm:tracking-[0.3em] font-mono">{slide.new.title}</h3>
                <ul className="space-y-2 sm:space-y-4">
                  {slide.new.points.map((pt, i) => (
                    <li key={i} className="flex items-center gap-2 sm:gap-4 text-sm sm:text-lg md:text-xl font-medium tracking-tight">
    ...
    case 'roadmap':
      return (
        <div className="flex-1 slide-wrapper overflow-y-auto sm:overflow-visible">
          <div className="slide-content-area">
            <h2 className="text-2xl sm:text-4xl md:text-5xl lg:text-7xl font-black mb-4 sm:mb-6 tracking-tighter text-center sm:text-left">Growth Roadmap</h2>
            {slide.subtitle && (
              <p className="text-sm sm:text-lg md:text-xl text-white/50 mb-6 sm:mb-8 leading-relaxed max-w-4xl text-center sm:text-left px-4">{slide.subtitle}</p>
            )}
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-8 pb-4 sm:pb-0">
              {slide.phases.map((ph, i) => (
                <motion.div
                  key={i}
                  whileHover={{ y: -8 }}
                  className="bg-white/[0.05] border border-white/[0.12] rounded-2xl sm:rounded-[32px] p-4 sm:p-8 md:p-12 flex flex-col h-full hover:border-accent/30 transition-colors"
                >
                  <div className="text-accent font-black text-3xl sm:text-4xl md:text-6xl mb-3 sm:mb-4 md:mb-5 opacity-40">{ph.phase}</div>
                  <h3 className="text-lg sm:text-xl md:text-3xl font-bold mb-1 sm:mb-2">{ph.name}</h3>
                  <div className="text-white/30 font-mono text-[9px] sm:text-[10px] md:text-xs tracking-[0.15em] sm:tracking-[0.2em] uppercase mb-3 sm:mb-4 md:mb-5">{ph.timeline}</div>
                  <p className="text-xs sm:text-sm md:text-base text-white/50 leading-relaxed mb-4 sm:mb-6 flex-1">{ph.desc}</p>
                  <div className="h-0.5 sm:h-1 w-6 sm:w-8 bg-accent/30 rounded-full" />
                </motion.div>
              ))}
            </div>
          </div>
    ...
    case 'grid':
      return (
        <div className="flex-1 slide-wrapper overflow-y-auto sm:overflow-visible">
          <div className="slide-content-area">
            <h2 className="text-2xl sm:text-4xl md:text-5xl lg:text-7xl font-black mb-4 sm:mb-6 tracking-tighter">{slide.title}</h2>
            {slide.subtitle && (
              <p className="text-sm sm:text-lg md:text-xl text-white/50 mb-8 sm:mb-16 leading-relaxed max-w-4xl px-4 sm:px-0">{slide.subtitle}</p>
            )}
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-12 pb-4 sm:pb-0">
              {slide.items.map((item, i) => (
                <div key={i} className="flex flex-col sm:flex-row gap-3 sm:gap-8 group cursor-pointer text-center sm:text-left">
                  <div className="w-10 h-10 sm:w-16 sm:h-16 mx-auto sm:mx-0 rounded-xl sm:rounded-2xl bg-white/[0.1] border border-white/[0.15] group-hover:border-accent/40 group-hover:bg-accent/10 flex items-center justify-center text-white/60 group-hover:text-accent flex-shrink-0 transition-all duration-300">
                    {React.cloneElement(item.icon, { size: 20, className: "sm:w-7 sm:h-7" })}
                  </div>
                  <div>
                    <h3 className="text-base sm:text-xl md:text-3xl font-bold mb-1 sm:mb-3 group-hover:text-accent transition-colors duration-300">{item.title}</h3>
                    <p className="text-xs sm:text-sm md:text-lg text-white/50 group-hover:text-white/70 leading-relaxed transition-colors duration-300">{item.desc}</p>
                  </div>
                </div>
              ))}
            </div>
    ...
    case 'mockup':
      return (
        <div className="flex-1 flex flex-col sm:flex-row max-h-full overflow-hidden sm:overflow-visible" style={{ padding: 0 }}>
          <div className="w-full sm:w-1/2 slide-wrapper flex flex-col justify-center">
            <div className="max-w-xl mx-auto sm:mx-0 text-center sm:text-left">
              <h2 className="text-3xl sm:text-6xl md:text-8xl font-black mb-4 sm:mb-10 tracking-tighter leading-[0.95] sm:leading-[0.8]">{slide.title}</h2>
              <p className="text-base sm:text-xl md:text-3xl font-medium text-white/50 leading-relaxed whitespace-pre-line">{slide.desc}</p>
            </div>
          </div>
    ...
    case 'feature':
      return (
        <div className="flex-1 slide-wrapper overflow-y-auto sm:overflow-visible">
          <div className="slide-content-area">
            <h2 className="text-2xl sm:text-4xl md:text-5xl lg:text-7xl font-black mb-4 sm:mb-6 tracking-tighter leading-tight">{slide.title}</h2>
            {slide.description && (
              <p className="text-sm sm:text-lg md:text-xl text-white/50 mb-4 sm:mb-8 leading-relaxed max-w-4xl px-4 sm:px-0">{slide.description}</p>
            )}
            <p className="text-base sm:text-xl md:text-2xl text-accent font-bold mb-6 sm:mb-10 uppercase tracking-wider sm:tracking-widest px-4 sm:px-0">{slide.subtitle}</p>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-10 pb-4 sm:pb-0">
              {slide.points.map((pt, i) => (
                <div key={i} className="flex items-center gap-3 sm:gap-5 bg-white/[0.06] p-4 sm:p-8 rounded-xl sm:rounded-2xl border border-white/[0.12] hover:border-accent/30 hover:bg-white/[0.08] transition-all cursor-pointer group">
                  <CheckCircle2 className="text-accent flex-shrink-0" size={18} />
                  <span className="text-sm sm:text-xl font-medium">{pt}</span>
                </div>
              ))}
            </div>
          </div>
    ...
    case 'split':
      return (
        <div className="flex-1 flex flex-col slide-wrapper overflow-y-auto sm:overflow-visible" style={{ paddingBottom: 0 }}>
          <div>
    ...
    case 'feature_highlight':
      return (
        <div className="flex-1 slide-wrapper">
          <div className="slide-content-area">
    ...
    case 'pricing':
      return (
        <div className="flex-1 slide-wrapper overflow-y-auto sm:overflow-visible">
          <div className="slide-content-area">
            <h2 className="text-2xl sm:text-5xl md:text-7xl font-black mb-4 sm:mb-6 tracking-tighter text-center">{slide.title}</h2>
            {slide.subtitle && (
              <p className="text-sm sm:text-lg md:text-xl text-white/50 mb-6 sm:mb-12 leading-relaxed text-center max-w-4xl mx-auto px-4">{slide.subtitle}</p>
            )}
      ...
      return (
        <div className="flex-1 slide-wrapper overflow-y-auto sm:overflow-visible">
          <div className="slide-content-area">
            ...
            <div className="max-w-4xl bg-gradient-to-br from-white/[0.05] to-transparent p-4 sm:p-12 rounded-2xl sm:rounded-[48px] border border-white/[0.1] mx-auto overflow-hidden">
              <div className="flex flex-col sm:grid sm:grid-cols-2 gap-6 sm:gap-x-12">
                <div className="space-y-2 sm:space-y-4">
                  {slide.benefits.map((b, i) => (
                    <div key={i} className="flex items-center gap-2 sm:gap-3">
                      <TrendingUp size={14} className="text-accent sm:w-[18px] sm:h-[18px] flex-shrink-0" />
                      <span className="text-xs sm:text-lg font-medium tracking-tight">{b}</span>
                    </div>
                  ))}
                </div>
    ...
    case 'quote':
      return (
        <div className="flex-1 slide-wrapper flex flex-col items-center justify-center text-center">
          <div className="slide-content-area">
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}>
              <p className="text-2xl sm:text-4xl md:text-6xl font-black mb-6 sm:mb-12 italic tracking-tighter leading-tight">
                {slide.text}
              </p>
              <div className="h-0.5 sm:h-1 w-16 sm:w-24 bg-accent mx-auto mb-4 sm:mb-6" />
              <p className="text-sm sm:text-xl font-bold uppercase tracking-[0.2em] sm:tracking-[0.4em] opacity-40">{slide.author}</p>
            </motion.div>
          </div>
        </div>
      );
    ...
    case 'cta':
      return (
        <div className="flex-1 slide-wrapper flex flex-col justify-center">
